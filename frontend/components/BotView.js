...
function CreateBot({ defaultSymbol, onCreated }) {
  ...
  const [dca, setDca] = useState({ base_order_usd: 100, safety_order_usd: 100, max_safety_orders: 5, price_deviation_pct: 2.5, take_profit_pct: 2 });
  ...
  const submit = async () => {
    ...
    try {
      const config = mode === "grid"
        ...
        : { base_order_usd: Number(dca.base_order_usd), safety_order_usd: Number(dca.safety_order_usd), max_safety_orders: Number(dca.max_safety_orders), price_deviation_pct: Number(dca.price_deviation_pct), take_profit_pct: Number(dca.take_profit_pct) };
    ...
    <div className="bg-bg-card border border-border rounded-xl p-4 space-y-3">
      ...
      <p className="text-[11px] text-tx-muted">
        {mode === "grid"
          ...
          : "Buys a base order, averages down with safety orders, takes profit on the whole deal above the average entry."}
      </p>
      ...
      <div className="flex items-center justify-between px-4 py-3 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <span className={clsx("text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase",
            bot.mode === "grid" ? "bg-brand-blue/15 text-brand-blue" : "bg-purple-500/15 text-purple-400")}>
            {bot.mode}
          </span>
          <div className="min-w-0">
            ...
            <div className="text-[11px] text-tx-muted">
              {bot.completed_trades} closed · {bot.mode === "grid" ? `${bot.open_levels} open levels` : `${bot.safety_used ?? 0} safety used`}
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <div className="flex items-center gap-1">
            {bot.status === "running"
              ? <button onClick={() => act(() => api.stopBot(bot.id))} className="text-[11px] px-2 py-1 rounded border border-border text-tx-muted hover:text-tx">Pause</button>
              : <button onClick={() => act(() => api.startBot(bot.id))} className="text-[11px] px-2 py-1 rounded border border-border text-emerald-400 hover:text-emerald-300">Resume</button>}
            <button onClick={() => act(() => api.deleteBot(bot.id))} className="text-[11px] px-2 py-1 rounded border border-border text-red-400 hover:text-red-300">Delete</button>
            <button onClick={() => setOpen(!open)} className="text-[11px] px-2 py-1 rounded border border-border text-tx-muted hover:text-tx">{open ? "Hide" : "Detail"}</button>
          </div>
        </div>
      </div>
      {open && (
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5">
      <div>
        ...
        <p className="text-xs text-tx-muted mt-1">
          Grid &amp; DCA bots simulated against real Binance price action — no API keys, no live orders.
